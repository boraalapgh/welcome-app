...
export function persistDashboardValues(values: Record<string, unknown>) {
  try {
    localStorage.setItem(STORAGE_KEY_DASHBOARD, JSON.stringify(values));
  } catch (e) {
    console.warn('Failed to persist dashboard values:', e);
  }
}
...
export function clearDashboardValues() {
  localStorage.removeItem(STORAGE_KEY_DASHBOARD);
}
