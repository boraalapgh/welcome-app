...
export default function Orb({
  ...
  const frag = /* glsl */ `
    ...
    vec4 draw(vec2 uv) {
      ...
      // Hard cutoff to remove outer glow/shadow
      // Original: float edgeCutoff = smoothstep(r0 * 1.02, r0 * 0.98, len);
      float edgeCutoff = smoothstep(r0 * 1.0, r0 * 0.95, len);
      vec4 result = extractAlpha(finalCol);
      // Kill any dark pixels outside the ring - if color is too dark, make it transparent
      float brightness = max(max(finalCol.r, finalCol.g), finalCol.b);
      float darkCutoff = smoothstep(0.05, 0.15, brightness);
      result.a *= edgeCutoff * darkCutoff;
